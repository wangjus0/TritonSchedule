...
Sets up this workspace for the Supabase migration without using MongoDB env vars.
...
validate_env_file() {
  [[ -f "$BACKEND_ENV" ]] || fail "Missing backend/.env. Copy backend/.env.example and fill Supabase values."
  ...
  if (( ${#found_prohibited[@]} > 0 )); then
    fail "Remove MongoDB env vars from backend/.env: ${found_prohibited[*]}. This migration uses Supabase."
  fi

  local required_supabase=(SUPABASE_URL SUPABASE_SERVICE_ROLE_KEY)
  local missing_supabase=()
  for key in "${required_supabase[@]}"; do
    local value
    value="$(env_value "$key" "$BACKEND_ENV")"
    if is_placeholder "$value"; then
      missing_supabase+=("$key")
    fi
  done

  if (( ${#missing_supabase[@]} > 0 )); then
    fail "Missing or placeholder required Supabase env values in backend/.env: ${missing_supabase[*]}"
  fi

  local recommended=(SUPABASE_PUBLISHABLE_KEY SUPABASE_ANON_KEY DATABASE_URL DB_PASSWORD API_KEY JWT_SECRET)
  ...
  log "Validated backend/.env uses required Supabase env vars and no MongoDB env vars."
}
...
ensure_frontend_env() {
  ...
  warn "Created frontend/.env from frontend/.env.example. Set VITE_API_KEY to match backend API_KEY before production builds."
}
